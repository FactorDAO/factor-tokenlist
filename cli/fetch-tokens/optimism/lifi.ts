...
async function main() {
  const endpoint = 'https://li.quest/v1/tokens?chains=OPT';
  const response = await fetch(endpoint);
  const data = await response.json();
  const lifiTokens = data.tokens['10'];
    ...
    try {
      ...
      if (checkToken) {
        ...
        if (token.logoURI) {
          if (
            !fs.existsSync(
              `./assets/optimism/${token.symbol.toUpperCase()}.png`,
            )
          ) {
  ...
  const entireList = tokens.map((token) => {
    if (fs.existsSync(`./assets/optimism/${token.symbol.toUpperCase()}.png`)) {
      token.logoUrl = `https://raw.githubusercontent.com/FactorDAO/factor-tokenlist/main/assets/optimism/${token.symbol.toUpperCase()}.png`;
    }
    return JSON.stringify(token);
  });
  const rawFile = compileFile(entireList);
  // Save the file
  fs.writeFileSync('./src/chains/optimism/general.ts', rawFile);
  exec('yarn format');
