...
import { tokens } from '../../../src/chains/arbitrum/morpho';
import fs from 'fs';
...
async function main() {
  ...
  for (const token of morphoTokens) {
    if (token.morphoBlue.chain.network === 'Arbitrum One') {
      ...
      console.log('👀 Token is not on arbitrum:', token.uniqueKey);
    }
  }
  ...
  // Save the file
  fs.writeFileSync('./src/chains/arbitrum/morpho.ts', rawFile);
  exec('yarn format');
