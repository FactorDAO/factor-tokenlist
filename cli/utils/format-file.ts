export const compileFile = (entireList: string[]) => {
  ...
  rawFile = rawFile.replace(/"CLAIM_REWARDS"/g, 'BuildingBlock.CLAIM_REWARDS');
  rawFile = rawFile.replace(/"YIELD"/g, 'BuildingBlock.YIELD');
  ...
  rawFile = rawFile.replace(/"CAMELOT"/g, 'Protocols.CAMELOT');
  rawFile = rawFile.replace(/"EULER"/g, 'Protocols.EULER');
